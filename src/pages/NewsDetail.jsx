...
import { db } from '../config/firebase'; // asegurate de tener tu archivo firebase.js
import { useAuth } from '../context/AuthContext'; // si tenés auth context
...
const handleSubmit = async (parentId = null) => {
  ...
  // Si parentId no es null ni string (o sea, es un evento), corregimos
...
const groupComments = (commentsArray) => {
  ...
  commentsArray.forEach(c => {
    if (c.parentId && commentMap[c.parentId]) {
      ...
      result.push(commentMap[c.id]); // comentario raíz
    }
  });
...
const Comment = ({ comment, isRoot = false }) => {
  ...
  return (
    <Box key={comment.id} p={isRoot ? 2 : 0} m={2} pl={2} border={ isRoot? "1px solid rgba(128, 128, 128, 0.1)" : 0} borderRadius={"20px"} >
      ...
      >
        {comment.likes?.includes(user?.uid) ? <ThumbUpAltIcon sx={{ color: "black" }}  /> : <ThumbUpOffAltIcon sx={{ color: "rgba(1, 1, 1, 0.36)" }} />}
        <Typography variant="body2" color="text.secondary" ml={1}>
          {comment.likes?.length || 0}  
        </Typography>
      </Button>

      <Button style={{color:"rgb(78, 78, 78)", textTransform:"lowercase"}} size="small" onClick={() => setIsReplying(true)}>Responder</Button>
      ...
      <Typography variant="caption" color="text.secondary">
        {comment.createdAt?.toDate
          ? `Hace ${formatDistanceToNow(comment.createdAt.toDate(), { addSuffix: false, locale: es })}`
          : 'Hace un momento'}
      </Typography>
